...
import com.cfrdocarmo.cfrfood.core.validation.ValidacaoException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<Object> handlePropertyBind(PropertyBindingException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {

        String path = joinPath(ex.getPath());

        ProblemType problemType = ProblemType.MENSAGEM_INCOMPREENSIVEL;
        String detail = String.format("A propriedade '%s' não existe no Objeto '%s'. Corrija ou remova essa propriedade e tente novamente.", path, ex.getReferringClass().getSimpleName() );
        Problem problem = createProblemBuilder(status, problemType, detail)
                .userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL)
                .build();
